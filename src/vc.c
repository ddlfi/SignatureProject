...
static tree_t create_tree(const paramset_t* params, unsigned int depth) {
  tree_t tree;
  uint32_t lambdaBytes = params->lambda / 8;
...
static void expand_seeds(tree_t* tree, const uint8_t* iv, const paramset_t* params) {
...
static tree_t generate_seeds(const uint8_t* rootSeed, const uint8_t* iv,
                             const paramset_t* params, unsigned int depth) {
  uint32_t lambdaBytes = params->lambda / 8;
