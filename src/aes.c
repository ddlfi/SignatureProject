...
int aes256_init_round_keys(aes_round_keys_t* round_key, const uint8_t* key) {
    return expand_key(round_key, key, KEY_WORDS_256, AES_BLOCK_WORDS,
                      ROUNDS_256);
}
...
uint8_t* aes_extend_witness(const uint8_t* key, const uint8_t* in,
    ...
    if (L_ke > 0) {
        ...
        for (unsigned int j = 0, ik = params->faest_param.Nwd; j < S_ke / 4;
             ++j) {
    ...
    // Step 10
    for (unsigned b = 0; b < beta;
         ++b, in += sizeof(aes_word_t) * block_words) {
        // Step 12
        aes_block_t state;
